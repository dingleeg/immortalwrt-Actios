...
const I18N = {
  zh: {
    ...
    building: "⏳ 正在触发编译...",
    ok_built: "✅ 编译已触发！预计 1.5~2 小时完成。<br>👉 <a href=\"{url}\" target=\"_blank\">点此查看编译进度 ↗</a><br>编译完成后在仓库 Releases 页面下载固件。",
  ...
  en: {
    ...
    ok_built: "✅ Build triggered! Estimated 1.5~2 hours.<br>👉 <a href=\"{url}\" target=\"_blank\">View build progress ↗</a><br>Download firmware from Releases when done.",
...
const PLUGIN_CATEGORIES = [
  {
    titleKey: "cat_proxy",
    plugins: [
      { pkg: "luci-app-openclash",      name: "OpenClash",        icon: "🐱", desc: { zh: "Clash 代理客户端",       en: "Clash proxy client" } },
      { pkg: "luci-app-passwall",       name: "PassWall",         icon: "🧱", desc: { zh: "多协议代理工具",         en: "Multi-protocol proxy" } },
      { pkg: "luci-app-passwall2",      name: "PassWall2",        icon: "🧱", desc: { zh: "PassWall 第二版",        en: "PassWall v2" } },
      { pkg: "luci-app-ssr-plus",       name: "SSR Plus+",        icon: "✈️", desc: { zh: "ShadowSocksR 增强版",   en: "ShadowSocksR enhanced" } },
  ...
  },
  {
    ...
    plugins: [
      ...
      { pkg: "luci-app-adguardhome",    name: "AdGuard Home",     icon: "🛡️", desc: { zh: "全网广告/追踪拦截",    en: "Full ad & tracker block" } },
  ...
  },
  {
    ...
    plugins: [
      { pkg: "luci-app-nlbwmon",        name: "带宽监控",         icon: "📈", desc: { zh: "网络带宽监视器",        en: "Bandwidth monitor" } },
      ...
      { pkg: "luci-app-netspeedtest",   name: "网速测试",         icon: "🚀", desc: { zh: "在线网速测试",          en: "Network speed test" } },
      { pkg: "luci-app-wrtbwmon",       name: "实时流量监测",     icon: "📡", desc: { zh: "实时流量监测",          en: "Real-time traffic monitor" } },
      { pkg: "luci-app-flow-offload",   name: "流量统计",         icon: "📊", desc: { zh: "流量统计",              en: "Traffic statistics" } },
    ]
  },
  {
    titleKey: "cat_tools",
    plugins: [
      { pkg: "luci-app-ttyd",           name: "TTYD 终端",        icon: "💻", desc: { zh: "网页版 SSH 终端",       en: "Web SSH terminal" } },
      ...
      { pkg: "luci-app-diskman",        name: "磁盘管理",         icon: "💾", desc: { zh: "硬盘分区挂载管理",      en: "Disk partition manager" } },
      ...
      { pkg: "luci-app-autoreboot",     name: "定时重启",         icon: "🔄", desc: { zh: "定时自动重启路由器",    en: "Scheduled reboot" } },
      ...
      { pkg: "luci-app-uhttpd",         name: "Web管理/uHTTPd",   icon: "🌐", desc: { zh: "Web 服务器配置",        en: "uHTTPd web server" } },
  ...
  },
  {
    ...
    plugins: [
      { pkg: "luci-app-ddns",           name: "DDNS",             icon: "🌐", desc: { zh: "动态域名解析",           en: "Dynamic DNS" } },
      ...
      { pkg: "luci-app-zerotier",       name: "ZeroTier",         icon: "🔗", desc: { zh: "虚拟局域网组网",         en: "Virtual LAN" } },
      ...
      { pkg: "luci-app-wireguard",      name: "WireGuard",        icon: "🔒", desc: { zh: "高性能 VPN 隧道",        en: "High-perf VPN tunnel" } },
      { pkg: "luci-app-frpc",           name: "FRP 客户端",       icon: "🚇", desc: { zh: "内网穿透客户端",         en: "Intranet tunnel client" } },
      ...
      { pkg: "luci-app-phtunnel",       name: "花生壳",           icon: "🥜", desc: { zh: "花生壳内网穿透",         en: "Oray tunnel" } },
      { pkg: "luci-app-pgyvpn",         name: "蒲公英组网",       icon: "🌼", desc: { zh: "蒲公英智能组网",         en: "Oray mesh VPN" } },
      ...
      { pkg: "luci-app-v2ray-server",   name: "V2ray 服务器",     icon: "🔀", desc: { zh: "V2Ray 服务端",           en: "V2Ray server" } },
      { pkg: "luci-app-tinyproxy",      name: "Tinyproxy",        icon: "🔌", desc: { zh: "轻量 HTTP 代理",         en: "Lightweight HTTP proxy" } },
      { pkg: "luci-app-polipo",         name: "Polipo",           icon: "🔌", desc: { zh: "HTTP 缓存代理",          en: "HTTP caching proxy" } },
      ...
      { pkg: "luci-app-kms-server",     name: "KMS 服务器",       icon: "🔑", desc: { zh: "Windows KMS 激活服务",   en: "KMS activation server" } },
      { pkg: "luci-app-jk-ac",          name: "集客AC控制器",     icon: "📡", desc: { zh: "集客无线AC控制器",       en: "JK AC controller" } },
      { pkg: "luci-app-ahcp",           name: "AHCP 服务器",      icon: "🌐", desc: { zh: "Ad-Hoc 配置协议",        en: "AHCP server" } },
    ]
  },
  {
    titleKey: "cat_sim",
    plugins: [
      { pkg: "luci-app-sms-tool",       name: "SMS Tool",         icon: "📨", desc: { zh: "收发短信管理",           en: "SMS management" } },
      { pkg: "luci-app-3ginfo-lite",    name: "3G/4G 信息",       icon: "📶", desc: { zh: "移动网络信号状态",       en: "Mobile signal status" } },
      { pkg: "luci-app-modemband",      name: "频段锁定",         icon: "📡", desc: { zh: "锁定 4G/5G 频段",        en: "Lock 4G/5G band" } },
  ...
  },
  {
    ...
    plugins: [
      ...
      { pkg: "luci-app-argon-config",   name: "Argon 主题设置",   icon: "🎨", desc: { zh: "Argon 主题配置面板",     en: "Argon theme settings" } },
  ...
  },
  {
    ...
    plugins: [
      ...
      { pkg: "luci-app-ftp",            name: "FTP 服务器",       icon: "📂", desc: { zh: "FTP 文件服务器",         en: "FTP server" } },
  ...
  },
  {
    titleKey: "cat_access",
    plugins: [
      { pkg: "luci-app-accesscontrol",  name: "上网时间控制",     icon: "⏱️", desc: { zh: "按设备限制上网时间",     en: "Per-device time control" } },
      ...
      { pkg: "luci-app-ip-mac-bind",    name: "IP/MAC 绑定",      icon: "🔗", desc: { zh: "IP 与 MAC 地址绑定",     en: "IP/MAC binding" } },
  ...
  },
  {
    titleKey: "cat_accel",
    plugins: [
      { pkg: "luci-app-turboacc",       name: "Turbo ACC",        icon: "🚀", desc: { zh: "软件流量加速",           en: "Software traffic acceleration" } },
      { pkg: "kmod-tcp-bbr",            name: "BBR",              icon: "🚀", desc: { zh: "TCP BBR 拥塞控制",       en: "TCP BBR congestion control" } },
  ...
  },
  {
    titleKey: "cat_docker",
    plugins: [
      { pkg: "luci-app-dockerman",      name: "Docker (Dockerman)", icon: "🐳", desc: { zh: "Docker 容器管理（推荐）", en: "Docker manager (recommended)" } },
      { pkg: "docker-ce",               name: "Docker CE",          icon: "🐳", desc: { zh: "Docker CE 容器引擎",     en: "Docker CE engine" } },
  ...
  },
  {
    titleKey: "cat_game",
    plugins: [
      { pkg: "luci-app-leigod-acc",     name: "雷神加速器",       icon: "⚡", desc: { zh: "雷神游戏加速器",         en: "Leigod game accelerator" } },
      { pkg: "luci-app-qiyou-acc",      name: "奇游联机宝",       icon: "🎮", desc: { zh: "奇游游戏加速",           en: "Qiyou game accelerator" } },
      { pkg: "luci-app-uugamebooster",  name: "UU 游戏加速",      icon: "🎮", desc: { zh: "网易 UU 游戏加速器",     en: "NetEase UU accelerator" } },
  ...
  },
  {
    ...
    plugins: [
      ...
      { pkg: "luci-app-xunlei",         name: "迅雷快鸟",         icon: "⬇️", desc: { zh: "迅雷快鸟宽带加速",       en: "Xunlei bandwidth boost" } },
...
async function triggerBuild() {
  ...
  if (ip)       inputs.extra_packages += (pkgs ? "," : "") + `__ip__${ip}`;
  if (hostname) inputs.extra_packages += `__hostname__${hostname}`;
  ...
  try {
    ...
    const workflow = wfData.workflows.find(w => w.path.includes("Build") || w.name.includes("Build"));
